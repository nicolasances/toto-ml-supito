...
def predict(request: Request, user_context: UserContext, exec_context: ExecutionContext): 
    ...
    # 1. Load the model
    persisted_model = PersistentNesuModel.load(exec_context)

    model = persisted_model.get_model()
    trained_encoders = persisted_model.get_trained_encoders()
    trained_scalers = persisted_model.get_trained_scalers()
